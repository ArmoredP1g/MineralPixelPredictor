...
class MCV_Split_Attn(nn.Module):
    def __init__(self, attn=False, sample_factor=5) -> None:
        ...
        self.weight_mask = nn.Parameter(torch.randn(4,1,19,19,1)/1e9)   # multiplied with: head(8), batch, row, col, dim
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.down_sampling_1(x.permute(0,3,1,2)))   # [batch, 19, 19, 384]
        x = torch.relu(self.down_sampling_2(x)).permute(0,2,3,1)   # [batch, 19, 19, 256]
        ...
        x = x*self.soft(self.weight_mask.reshape(4,1,19*19,1)).reshape(4,1,19,19,1)  # [4, b, 19, 19, 32]
